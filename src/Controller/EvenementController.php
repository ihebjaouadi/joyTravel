...
//update an event
    ...
    /**
     * @Route("/u/TriejsonDESC", name="TriPriceDESCUsessr", methods={"GET"})
     */
    public function TrieDESCjson(EvenementRepository $evenementRepository,NormalizerInterface $Normalizer)
    {
        //$evenement = $evenementRepository->findAll();
        $evenement = $evenementRepository->OrderByPriceDESC();
        $jsonContent = $Normalizer->normalize($evenement, 'json', ['groups' => 'post:read']);
        return new Response(json_encode($jsonContent));
    }

    /**
     * @Route("/u/TriejsonASC", name="TriPriceASCUsessr", methods={"GET"})
     */
    public function TrieASCjson(EvenementRepository $evenementRepository,NormalizerInterface $Normalizer)
    {
        //$evenement = $evenementRepository->findAll();
        $evenement = $evenementRepository->OrderByPriceASC();
        $jsonContent = $Normalizer->normalize($evenement, 'json', ['groups' => 'post:read']);
        return new Response(json_encode($jsonContent));
    }
